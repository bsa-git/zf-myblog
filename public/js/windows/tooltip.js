...
TooltipManager = {
    ...
    onBeforeShow: function(eventName, win) {
        ...
        var left = parseFloat(win.getLocation().left);
        var dim = win.element.getDimensions();
        
        var scrollTop = TooltipManager._getScrollTop();
        var pageHeight = TooltipManager._getPageHeight();
        
        if (top + dim.height > scrollTop + pageHeight) {
//            var position = Position.cumulativeOffset(this.element);
            // При использовании "this.element" появлялась ошибка 
            // пришлось использовать "win.element" 
            //========== mychange-begin =========//
//            var position = Position.cumulativeOffset(win.element);
            //========== mychange-end =========//
            
//            var top = position[1] - TooltipManager.options.shiftY - dim.height;
//            win.setLocation(top, position[0] + TooltipManager.options.shiftX)
            // Изменил вычисление позиции newTop
            //========== mychange-begin =========// 
//            var newTop = position[1] + TooltipManager.options.shiftY + 10;
//            var newLeft = position[0] + TooltipManager.options.shiftX;
            if((top - dim.height) > 0){
                shiftY = TooltipManager.options.shiftY + top - dim.height;
            }else{
                shiftY = TooltipManager.options.shiftY;
            }
            
            var shiftX = TooltipManager.options.shiftX + dim.width + 10;
            var newTop = scrollTop + shiftY;
            var newLeft = left - shiftX;
