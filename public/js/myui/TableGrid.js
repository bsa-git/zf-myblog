...
MY.TableGrid = Class.create({
    ...
    initialize: function (tableModel) {
        ...
        //========== mychange-begin =========//
        // Установим признак создания меню отчетов
        this.addReportMenuFlg = (this.options.addReportMenu) ? true : false;
        //========== mychange-end =========//
        ...
        for (var i = 0; i < this.columnModel.length; i++) {
            if (!this.columnModel[i].hasOwnProperty('editor'))
                this.columnModel[i].editor = new MY.TextField();
            ...
            if (!this.columnModel[i].hasOwnProperty('visible'))
                this.columnModel[i].visible = true;
            if (!this.columnModel[i].hasOwnProperty('sortable'))
                this.columnModel[i].sortable = true;
            if (!this.columnModel[i].hasOwnProperty('type'))
                this.columnModel[i].type = 'string';
            if (!this.columnModel[i].hasOwnProperty('selectAllFlg'))
                this.columnModel[i].selectAllFlg = false;
            if (!this.columnModel[i].hasOwnProperty('sortedAscDescFlg'))
                this.columnModel[i].sortedAscDescFlg = 'DESC';
        ...
        this.gapWidth = 21;//21
        if (Prototype.Browser.WebKit) {
            this.gapWidth = this.hb.getGapWidth();
        }
        //========== mychange-end =========//
    },
    ...
    render: function (target) {
        ...
        this.bodyDiv.observe('dom:dataLoaded', function () {
            ...
            if (self.addDraggingBehaviorFlg)
                self._makeAllColumnDraggable();
            if (self.addSettingBehaviorFlg)
                self._applySettingMenuBehavior();
            //========== mychange-begin =========//
            // установим события для меню отчетов
            if (self.addReportMenuFlg)
                self._applyReportMenuBehavior();
            //========== mychange-end =========//
        ...
        if (this.options.toolbar) {
            ...
            //========== mychange-begin =========//
            // Добавление событий на пользовательские кнопки
            var elements_user = this.options.toolbar.elements_user || [];
            elements_user.each(function (element_user) {
                Event.observe($(element_user + '_' + id), 'click', function () {
                    if (self.options.toolbar['on' + element_user]) {
                        self.options.toolbar['on' + element_user].call();
                    }
                });
            })
            //========== mychange-end =========//
    ...
    _createTableLayout: function () {
        ...
        if (this.options.toolbar) {
            ...
            if (elements.indexOf(MY.TableGrid.ADD_BTN) >= 0) {
                if (beforeFlg)
                    html[idx++] = '<div class="toolbar-separator">&#160;</div>';
            ...
            elements_user.each(function (element_user) {
                if (beforeFlg)
                    html[idx++] = '<div class="toolbar-separator">&#160;</div>';
                html[idx++] = '<a href="#" class="toolbar-button" id="' + element_user + '_' + id + '"><span class="icon ' + element_user + '">&nbsp;</span><span class="text">' + i18n.getMessage('label.' + element_user) + '</span></a>';
            })
        ...
        html[idx++] = '<div id="mtgHRB' + id + '" style="position:relative;padding:0;margin:0;width:' + (this.headerWidth + this.gapWidth) + 'px;height:' + this.headerHeight + 'px;">';
        ...
        if (this.options.title)
            this.bodyHeight = this.bodyHeight - this.titleHeight - 1;
        if (this.options.pager)
            this.bodyHeight = this.bodyHeight - this.pagerHeight - 1;
        if (this.options.toolbar)
            this.bodyHeight = this.bodyHeight - this.toolbarHeight - 1;
        ...
        //========== mychange-begin =========//
        // создадим меню отчетов в дереве DOM
        if (this.addReportMenuFlg) {
            html[idx++] = '<div id="mtgRB' + id + '" class="my-tablegrid-setting-button" style="left:' + (this.tableWidth - 60) + 'px"><div class="icon-print">&nbsp;</div></div>';
            // Adding Table Report Menu
            html[idx++] = this._createReportMenu();
        }
        //========== mychange-end =========//
    ...
    _createTableBody: function (rows) {
        ...
        if (lastRowToRender > rows.length)
            lastRowToRender = rows.length;
                ...
                var elementClickHandler = (function (editor, element, innerElement) {
                    return function () {
                        if (editor.selectable == undefined || !editor.selectable) {
                            ...
                            if (editor.hasOwnProperty('getValueOf'))
                                value = editor.getValueOf(element.checked);
                            self.setValueAt(value, x, y, false);
                            if (y >= 0 && self.modifiedRows.indexOf(y) == -1)
                                self.modifiedRows.push(y); //if doesn't exist in the array the row is registered
                        }
                        if (editor.onClickCallback)
    ...
    //========== mychange-begin =========//
    /**
     * Creates the Report Menu
     */
    _createReportMenu: function () {
        var id = this._mtgId;
        var cm = this.columnModel;
        var elementsReportMenu = this.options.toolbar.elementsReportMenu || [];
        //----------------------------
        var bh = this.bodyHeight + 30;
        var height = (elementsReportMenu.length * 25 > bh) ? bh : 0;
        var html = [];
        var idx = 0;
        if (height > 0)
            html[idx++] = '<div id="mtgRM' + id + '" class="my-tablegrid-menu shadow" style="height:' + height + 'px">';
        else
            html[idx++] = '<div id="mtgRM' + id + '" class="my-tablegrid-menu shadow">';
        html[idx++] = '<ul>';

        elementsReportMenu.each(function (element) {
            html[idx++] = '<li>';
            html[idx++] = '<a href="#" class="my-tablegrid-menu-item">';
            html[idx++] = '<table border="0" cellpadding="0" cellspacing="0" width="100%">';
            html[idx++] = '<tr><td width="25"><div class="icon ' + element + '">&nbsp;</div></td>';
            html[idx++] = '<td><span class="text" id="' + element + '_' + id + '">' + i18n.getMessage('label.' + element) + '</span></td></tr>';
            html[idx++] = '</table>';
            html[idx++] = '</a>';
            html[idx++] = '</li>';
        })
    ...
    /**
     * Applies Report Menu behavior
     */
    _applyReportMenuBehavior: function () {
        var self = this;
        var settingMenu = $('mtgRM' + this._mtgId);
        var settingButton = $('mtgRB' + this._mtgId);
        ...
        var elementsReportMenu = this.options.toolbar.elementsReportMenu || [];
        elementsReportMenu.each(function (element) {
            Event.observe($(element + '_' + self._mtgId), 'click', function () {
                settingMenu.setStyle({
                    visibility: 'hidden'
                });
                if (self.options.toolbar['on' + element]) {
                    self.options.toolbar['on' + element].call();
                }
            });
        })
    },
    ...
    _makeAllColumnsResizable: function () {
        ...
        var topPos = 0;
        if (this.options.title)
            topPos += this.titleHeight;
        if (this.options.toolbar)
            topPos += this.toolbarHeight;
        var columnIndex;
        var self = this;
        var leftPos = 0;
        ...
        new Draggable(self.resizeMarkerRight, {
            ...
            onStart: function () {
                ...
                if (self._hasHScrollBar())
                    markerHeight = markerHeight - scrollBarWidth + 1;
    ...
    _resizeColumn: function (index, newWidth) {
        ...
        $('mtgHRT' + id).width = this.headerWidth + this.gapWidth;
    ...
    _detectDroppablePosition: function (columnPos, width, dragColumn, index) {
        ...
        for (var i = 0; i < cm.length; i++) {
            if (cm[i].visible)
                sepLeftPos += parseInt(cm[i].width) + gap;
    ...
    _moveColumn: function (fromColumnId, toColumnId) {
        ...
        if (fromColumnId == null
                ...
                || (toColumnId + 1 == fromColumnId && fromColumnId == this.columnModel.length - 1))
            return;
        ...
        if (toColumnId > 0 && toColumnId < fromColumnId)
            targetColumnId++;
        if (targetColumnId == fromColumnId)
            targetColumnId--;
        for (var c = 0; c < columnModelLength; c++) {
            if (c == targetColumnId)
                aTemp[k++] = columnModelEntry;
        ...
        if (fromColumnId == this.sortedColumnIndex)
            this.sortedColumnIndex = toColumnId;
    },
    /**
                ...
                var f_blur = (function (x, y, element) {
                    return function () {
                        if (self._blurCellElement(element))
                            self.editedCellId = null;
                        if (self.onCellBlur)
                            self.onCellBlur(element, row[x], x, y, cm[x].id);
                    };
                })(i, j, element);
    ...
    _editCellElement: function (element) {
        ...
        if (isInputFlg) {
            ...
            if (editor.selectable == undefined || !editor.selectable) {
                value = input.checked;
                if (editor.hasOwnProperty('getValueOf'))
                    value = editor.getValueOf(input.checked);
                this.setValueAt(value, x, y, false);
                if (y >= 0 && this.modifiedRows.indexOf(y) == -1)
                    this.modifiedRows.push(y); //if doesn't exist in the array the row is registered
            }
    ...
    _blurCellElement: function (element) {
        ...
        if (isInputFlg) {
            if (editor.hide)
                editor.hide(); // this only happen when editor is a Combobox
            if (editor instanceof MY.DatePicker && editor.visibleFlg)
                return false;
            if (editor.reset)
                editor.reset();
            /*
        ...
        if (y >= 0 && this.rows[y][columnId] != value) {
            ...
            //===== mychange-begin ==========
            // Это изменение сделано, т.к. не верно отрабатывает MY.ComboBox
            if (editor instanceof MY.ComboBox) {
            } else {
                innerElement.addClassName('modified-cell');
                if (this.modifiedRows.indexOf(y) == -1)
                    this.modifiedRows.push(y); //if doesn't exist in the array the row is registered
            }
            //===== mychange-end ==========//
        } else if (y < 0) {
            ...
            element.observe('mousemove', function () {
                ...
                if (sortable || editor == 'checkbox' || editor instanceof MY.TableGrid.CellCheckbox) {
                    ...
                    sortAscMenuItem.observe('click', function () {
                        //====== mychange-begin =======
                        // Изменения для правильной отработки сортировки в таблице
                        if (self._indAscDescFlg == 'ASC-' + selectedHCIndex) {
                            return;
                        }
                        //====== mychange-end =======
                    ...
                    sortDescMenuItem.observe('click', function () {
                        //====== mychange-begin =======
                        // Изменения для правильной отработки сортировки в таблице
                        if (self._indAscDescFlg == 'DESC-' + selectedHCIndex) {
                            return;
                        }
                        //====== mychange-end =======
        ...
        headerButton.observe('click', function () {
            ...
            if (headerButtonMenu.getStyle('visibility') == 'hidden') {
                ...
                if (self.renderedRows > 0
                    ...
                    var selectAllHandler = function () { // onclick handler
                        ...
                        if (cm[selectedHCIndex].editor instanceof MY.TableGrid.CellCheckbox
                                && cm[selectedHCIndex].editor.selectable)
                            selectableFlg = true;
                        var renderedRows = self.renderedRows;
                        var beginAtRow = 0;
                        if (self.newRowsAdded.length > 0)
                            beginAtRow = -self.newRowsAdded.length;
                        var x = selectedHCIndex;
                        for (var y = beginAtRow; y < renderedRows; y++) {
                            var element = $('mtgInput' + id + '_' + x + ',' + y);
                            var value = element.checked = flag;
                            if (!selectableFlg) {
                                if (cm[x].editor.hasOwnProperty('getValueOf'))
                                    value = cm[x].editor.getValueOf(element.checked);
                                self.setValueAt(value, x, y, false);
                                // if doesn't exist in the array the row is registered
                                if (y >= 0 && self.modifiedRows.indexOf(y) == -1)
                                    self.modifiedRows.push(y);
                            }
                        }
                ...
                if (self.options.title)
                    topPos += self.titleHeight;
                if (self.options.toolbar)
                    topPos += self.toolbarHeight;
    ...
    _sortData: function (idx, ascDescFlg) {
        ...
        if (cm[idx].sortable) {
            ...
            $('mtgSortIcon' + id + '_' + this.sortedColumnIndex).setStyle({
                visibility: 'hidden'
            });
            $('mtgIHC' + id + '_' + this.sortedColumnIndex).setStyle({
                color: 'dimgray'
            });
            $('mtgSortIcon' + id + '_' + idx).setStyle({
                visibility: 'visible'
            });
            $('mtgIHC' + id + '_' + idx).setStyle({
                color: 'black'
            });
            ...
            //====== mychange-begin =======
            // Изменения для правильной отработки сортировки в таблице
            this._indAscDescFlg = ascDescFlg + '-' + idx
            //====== mychange-end =======
        }
    },
    ...
    _toggleColumnVisibility: function (index, visibleFlg) {
        ...
        headerRowTable.width = this.headerWidth + this.gapWidth;
    ...
    _retrieveDataFromUrl: function (pageNumber, firstTimeFlg) {
        ...
        if (this.pager != null && this.pager.pageParameter)
            pageParameter = this.pager.pageParameter;
        this.request[pageParameter] = pageNumber;
        //========= mychange-begin ===========//
        // Обновление параметра запроса - joinTableForSort
        if (this.tableModel.updateRequestForSort) {
            this.tableModel.updateRequestForSort.call();
        }
        //========= mychange-end ==========//
        ...
        new Ajax.Request(this.url, {
            ...
            onSuccess: function (response) {
                ...
                //========= mychange-begin ===========//
                // Если появляется неизвестное сообщение, то выведем его
                if (self.tableModel.onFailure && tableModel.unexpected_message) {
                    self.tableModel.onFailure.call(null, tableModel.unexpected_message);
                }
                //========= mychange-end ==========//
                ...
                try {
                    ...
                    self.pager = null;
                    if (tableModel.options != null && tableModel.options.pager)
                        self.pager = tableModel.options.pager;
                    if (self.pager == null)
                        self.pager = {};
    ...
    _updatePagerInfo: function (emptyFlg) {
        ...
        if (this.pager.total > 0) {
            ...
            //========= mychange-begin ===========//
            // Удалим инф. с какой по какую страницу отображается
            // это связано - при разрешении 1024х768 не вмещается надпись

            if (pager.from && pager.to) {
                var temp = i18n.getMessage('message.rowsDisplay', {
                    from: pager.from,
                    to: pager.to
                });
            }

            temp += i18n.getMessage('message.totalDisplay', {
                total: pager.total
            });
            ...
            if (pager.pages) {
                ...
                temp = i18n.getMessage('message.pagePrompt', {
                    pages: pager.pages,
                    input: input
                });
            ...
            //========= mychange-begin ===========//
            // добавим в нижнем колонтитуле кол. записей на странице 
            var rowsByPage = 10;
            if (this.request.rowsByPage) {
                if (this.request.rowsByPage == -1) {
                    rowsByPage = i18n.getMessage('label.allRows');
                } else {
                    rowsByPage = this.request.rowsByPage;
                }

            }
            var input = '<input type="text" name="mtgRowsByPageInput' + id + '" id="mtgRowsByPageInput' + id + '" value="' + rowsByPage + '" class="my-tablegrid-page-input" size="3" maxlength="3">';
            temp = i18n.getMessage('message.rowsByPage', {
                input: input
            });
    ...
    _addPagerBehavior: function () {
        ...
        if (total > 0) {
            ...
            //========= mychange-begin ===========//
            // определим событие на изменение кол. записей на стр.
            // -1 -> это все записи на странице
            var keyHandler_RowsByPage = function (event) {
                if (event.keyCode == Event.KEY_RETURN) {
                    var rowsByPage = $('mtgRowsByPageInput' + id).value;
                    rowsByPage = parseInt(rowsByPage, 10);
                    if (rowsByPage) {

                        if (rowsByPage == 0 || rowsByPage < -1) {
                            rowsByPage = this.request.rowsByPage;
                        }
                        if (rowsByPage == -1) {
                            $('mtgRowsByPageInput' + id).value = i18n.getMessage('label.allRows');
                        } else {
                            $('mtgRowsByPageInput' + id).value = rowsByPage;
                        }
                        // если ввели новое кол. строк на стр., то перезагрузим таблицу
                        if (this.request.rowsByPage !== rowsByPage) {
                            this.request.rowsByPage = rowsByPage;
                            self._retrieveDataFromUrl.call(self, currentPage);
                        }
                    } else {
                        if (this.request.rowsByPage == -1) {
                            $('mtgRowsByPageInput' + id).value = i18n.getMessage('label.allRows');
                        } else {
                            $('mtgRowsByPageInput' + id).value = this.request.rowsByPage;
                        }
                    }
                }
            };
            $('mtgRowsByPageInput' + id).observe('keydown', keyHandler_RowsByPage.bindAsEventListener(this));
    ...
    resize: function () {
        ...
        this.tableWidth = parseInt(width) - 2;
        var tallerFlg = false;
        if ((parseInt(height) - 2) > this.tableHeight)
            tallerFlg = true;
        ...
        //========== mychange-begin =========//
        // Изменим абсолютное положение кнопки при изменении размера окна
        settingButton = $('mtgRB' + this._mtgId);
        if (settingButton) {
            settingButton.setStyle({
                left: (this.tableWidth - 60) + 'px'
            });
        }
        //========== mychange-end =========//
        ...
        if (this.options.pager)
            this.bodyHeight = this.bodyHeight - this.pagerHeight;
        if (this.options.toolbar)
            this.bodyHeight = this.bodyHeight - this.pagerHeight;
        ...
        if (idx >= 0) {
            ...
            if (newRowsAdded.length > 0) { // there are new rows added
                for (j = 0; j < newRowsAdded.length; j++) {
                    y = -(j + 1);
                    if ($('mtgInput' + id + '_' + idx + ',' + y).checked)
                        result.push(y);
                }
            }
            ...
            for (j = 0; j < renderedRows; j++) {
                y = j;
                if (this.deletedRows.indexOf(this.getRow(y)) == -1 && $('mtgInput' + id + '_' + idx + ',' + y) != null && $('mtgInput' + id + '_' + idx + ',' + y).checked)
                    result.push(y);
            }
    ...
    highlightRow: function (id, value) {
        ...
        for (var i = 0; i < this.rows.length; i++) {
            ...
            //---- mychange-begin -----
            // изменение введено для правильной подсветки строки
            if (this.rows[i][id] == value) {
                //---- mychange-end -----
...
var HeaderBuilder = Class.create({
    initialize: function (id, cm) {
        this.columnModel = cm;
        this._mtgId = id;
        this.gap = 2; //2 diff between width and offsetWidth
        if (Prototype.Browser.WebKit) {
            this.gap = 0;
        }
        ...
        if (Prototype.Browser.WebKit) {
            this.gap = 0;
            //            var _gapWidth = this._getColumnCount() * 4.444;
            this.gapWidth = this.getGapWidth();//Math.round(_gapWidth);//this._getColumnCount() * 3;
        }
    },
    getGapWidth: function () {
        ...
        switch (columnCount) {
            ...
            default:
                ...
                if (columnCount > 9 && columnCount < 19) {
                    gapWidth = 50;
                }
                if (columnCount > 19) {
                    gapWidth = 70;
    ...
    _createHeaderRow: function () {
        ...
        html[idx++] = '<table id="mtgHRT' + id + '" width="' + (this.headerWidth + this.gapWidth) + '" cellpadding="0" cellspacing="0" border="0" class="my-tablegrid-header-row-table">';
        html[idx++] = '<thead>';
    ...
    _getHeaderRow: function (nl, elements, column) {
        ...
        if (nl > 0)
            result = this._getHeaderRow(--nl, result);
        return result;
    },
    /**
    ...
    _getStartingPosition: function () {
        var result = 0;
        while (true) {
            if (this.filledPositions.indexOf(result) == -1)
                break;
            result++;
        }
        return result;
