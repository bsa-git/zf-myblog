...
MY.Autocompleter = Class.create(MY.TextField, {
    ...
    onClick : function(event) {
        ...
        this.hide();
        //===== mychange-begin =======
        if (this.options.onClickItem) {
            this.options.onClickItem.call();
        }
        //===== mychange-end =======
    },
    ...
    _getEntry : function(index) {
        ...
        // Иногда индех выбирается не корректно, 
        // index > countNodes в списке, поэтому значение в списке не выбирается
        // и происходит ошибка в ф-ии updateElement -> value = Element.collectTextNodesIgnoreClass(selectedElement, 'informal');
        // где selectedElement = не определена
        if(!this.update){
            return undefined;
        }
        var countNodes = this.update.firstChild.childNodes.length;
        if(countNodes > 0 && index > countNodes){
            index = countNodes - 1;
        }
        //======= mychange-end ==========//
    ...
    _syncScroll : function(entry, bottomFlg) {
        ...
        //======= mychange-begin ==========//
        // изменения для совместимости с IE
        if(entry === undefined){
            return;
        }
        //======= mychange-end ==========//
