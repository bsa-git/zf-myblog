...
BSA.iniAdminPostTags = function()
{
    ...
    var tableModel = {
        ...
        {
            id : 'tag',
            title : lb.getMsg('columnPostTag'),
            ajaxdata : self.ajaxData.selfFieldTable,
            joinTable: 'blog_posts_tags',
            required : true,
            width : 200,
            editable: true,
            editor: new MY.ComboBox({
                items: self.itemsComboBox.get('tag'),
                validate : function(value, errors) {
                    self._validateValue(
                    {
                        table: self.name,
        ...
        {
            id : 'label',
            title : lb.getMsg('columnPostTagLabel'),
            ajaxdata : self.ajaxData.selfFieldTable,
            joinTable: 'blog_posts_tags',
            required : true,
            width : 200,
            ...
            editor: new MY.ComboBox({
                items: self.itemsComboBox.get('label'),
                validate : function(value, errors) {
                    self._validateValue(
                    {
                        table: self.name,
